"""单查询检测：稠密向量（bge）单路检索的 mMARCO 检索质量评测。

与 test_embedding.py 同思路（仅用稠密向量），但数据源换成 chinese_collection.tsv，
评测指标：MRR@10 / Recall@1 / Recall@10 / nDCG@10，并与官方 BM25 run 基线对照。

复用 mmarco-hybrid 集合里的 dense 字段做检索（避免 880 万条二次向量化），
...
class MmarcoDenseEval(unittest.TestCase):
    def setUp(self):
    ...
    def test_dense_retrieval_quality(self):
        metrics = compute_metrics(
            self.queries, self.qrels, lambda qid, q: dense_search(self.store, q, K), K
        )
        ...
        print_metrics("稠密单路检索（dense-only）", metrics)
